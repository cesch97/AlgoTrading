...
include("src/trad_sys_creation.jl")
...
function main()
    ...
    if persist
        open("train_trad_sys-persist.sh", "w") do io
            write(io, "#!/bin/bash\n")
            ...
            screen_cmd = "screen -dm $(ENV["HOME"])/julia-1.5.0/julia ./train_trad_sys.jl"
    ...
    if !isnothing(parsed_args["conf_file"])
        ...
        conf_file = YAML.load_file("./configs/trad_sys_creation.yml")
    ...
    while true
        try
            trad_sys_creation(conf_file)
            break
        catch e
            open("./logs/train_trad_sys-log.txt", "a") do io
    ...
    rm("train_trad_sys-persist.sh", force=true)
