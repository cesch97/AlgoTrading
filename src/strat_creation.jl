...
function strat_creation(config_file)
    if isa(config_file, String)
        config = YAML.load_file(config_file)
        log_dir = config["log_dir"]
    else
        config = config_file
        log_dir = config["strats_dir"]
    end    
    ...
    while !isnothing(iter_state)
        ...
        if !(cfg_file_path in readdir("$log_dir/configs", join=true))
            ...
            evo_ps_dict["log_dir"] = "$log_dir/strategies"
        ...
        score = evolve_strategy(cfg_file_path)[2]
